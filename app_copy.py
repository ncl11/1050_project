...
def static_stacked_trend_graph(stack=False):
    ...
    df = df[df['Release'] == 'The War with Grandpa']
    if df is None:
        return go.Figure()
    ...
    fig.add_trace(go.Scatter(x=x, y=df['Daily'], mode='lines', name='The wwg',
                             line={'width': 2, 'color': COLORS[1]},
                             stackgroup='stack' if stack else None))
    fig.add_trace(go.Scatter(x=x, y=df['google trends'], mode='lines', name='GT',
...
def static_stacked_trend_graph2(stack=False):
    """
    Returns scatter line plot of all power sources and power load.
    If `stack` is `True`, the 4 power sources are stacked together to show the overall power
    production.
